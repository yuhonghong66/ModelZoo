...
python -u vgg_neon.py --test_only -i ${EXECUTOR_NUMBER} -w /data/i1k-extracted -vvv --manifest_root /data/i1k-extracted --manifest val:/data/i1k-extracted/val-index.csv --model_file $WEIGHTS_FILE --no_progress_bar -z 16 --vgg_version E | tee output.dat 2>&1
rc=$?
if [ $rc -ne 0 ];then
    exit $rc
fi
